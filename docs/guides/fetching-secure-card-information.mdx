...
2. Call the GET url specified in the `callbackUrl` response body field. The `callbackUrl` can only be used a single time and once used, subsequent calls to this url will return a `403` response code.
